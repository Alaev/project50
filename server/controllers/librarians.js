...
module.exports = {
  ...
  replaceLibrarian: async(req, res) => {
    ...
    const result = await Librarian.findOneAndUpdate({
      ID: librarianID
    }, newLibrarian);

    if (!result) {
      res.status(404).json({
        message: "Can't replaced librarian, librarian was not found"
      });
    } else {
      res.status(200).json({
        message: 'Great news, librarian was replaced'
      });
    }
  },
  ...
  deleteLibrarian: async(req, res) => {
    ...
    const removed = await Librarian.findOneAndRemove({
      ID: librarianID
    });

    if (!removed) {
      res.status(200).json({
        message: "Can't delete librarian, librarian was not found"
      });
    } else {
      res.status(200).json({
        message: 'Librarian was deleted!'
      });
    }
  }
};
